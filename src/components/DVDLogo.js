...
export default function DVDLogo() {
  ...
  const [dvdLogoPosition, setDvdLogoPosition] = useState({
    x: getRandomNumber(0, window.innerWidth - 200),
    y: getRandomNumber(0, window.innerHeight - 97.45),
    xSpeed: 1,
    ySpeed: 1,
  });

  const { r, g, b } = dvdLogoColor;
  const { x, y, xSpeed, ySpeed } = dvdLogoPosition;

  function moveLogo() {
    setDvdLogoPosition((dvdLogoPosition) => ({
      ...dvdLogoPosition,
      x: x + xSpeed,
      y: y + ySpeed,
    }));
